...
class SampleController extends Controller
{
    ...
    {
        return SampleResource::collection($action->execute($request->validated(), $request->user()));
    }
    ...
    public function update(UpdateSampleRequest $request, Sample $sample, UpdateSampleAction $action): SampleResource
    {
        $this->authorize('update', $sample);

        return new SampleResource($action->execute($sample, $request->validated(), $request->user()));
    }
    ...
    public function destroy(Sample $sample, DeleteSampleAction $action): JsonResponse
    {
        ...
        $action->execute($sample, request()->user());
