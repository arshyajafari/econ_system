...
class ReserveInventoryAction {
    public function execute(OrderItem $orderItem): Collection {
        $requiredQuantity = $orderItem->fulfillmentQuantity();

        if ($requiredQuantity <= 0) throw new BusinessRuleException('مقدار قابل تحویل سفارش باید بیشتر از صفر باشد.');
        ...
        if ($availableQuantity < $requiredQuantity) throw new BusinessRuleException('موجودی قابل رزرو برای محصول کافی نیست.');
        ...
        foreach ($batches as $batch) {
            ...
            $batch->save();
            $allocations->push(['inventory_batch_id' => $batch->id, 'quantity' => $reserveQuantity]);
            $remaining -= $reserveQuantity;
        }

        if ($remaining > 0) throw new BusinessRuleException('رزرو کامل موجودی سفارش انجام نشد.');
        return $allocations;
    }
}
