...
class ReceiveInventoryAction
{
    public function execute(array $data): InventoryBatch
    {
        return DB::transaction(function () use ($data) {
            ...
            if ($purchasePrice < 0) {
                throw new BusinessRuleException('قیمت خرید نمی‌تواند منفی باشد.');
            }
            ...
            if ($batchNumber !== null) {
                $batch = InventoryBatch::query()
                    ...
                    ->where(function ($query) use ($expireDate) {
                        if ($expireDate === null) $query->whereNull('expire_date');
                        else $query->whereDate('expire_date', $expireDate);
                    })
                    ->lockForUpdate()
                    ->first();
            }
            ...
            if ($batch) {
                ...
                $oldPurchasePrice = (float) ($batch->purchase_price ?? 0);
                $weightedPrice = $oldQuantity > 0
                    ? (($oldQuantity * $oldPurchasePrice) + ($quantity * $purchasePrice)) / $newQuantity
                    : $purchasePrice;
            ...
            return $batch->fresh(['product.currentPrice', 'movements']);
        });
    }
}
