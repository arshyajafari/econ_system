        ...
        public function execute(Invoice $invoice): Invoice {
            return DB::transaction(function () use ($invoice) {
                ...
                if ($invoice->status !== InvoiceStatus::DRAFT) {
                    throw new RuntimeException('فقط فاکتور در وضعیت draft قابل صدور است.');
                }

                if ($invoice->items->isEmpty()) {
                    throw new RuntimeException('فاکتور باید حداقل یک آیتم داشته باشد.');
                }
                ...
                if ($expectedTotal <= 0) {
                    throw new RuntimeException('مبلغ نهایی فاکتور باید بیشتر از صفر باشد.');
                }

                if (round((float)$invoice->total_amount, 2) !== round($expectedTotal, 2)) {
                    throw new RuntimeException('مبلغ نهایی فاکتور با اقلام فاکتور مطابقت ندارد.');
                }
