    ...
    public function execute(array $data, User $user): OrderReturn {
        return DB::transaction(function () use ($data, $user) {
            ...
            foreach ($requestedQuantities as $orderItemPublicId => $requested) {
                ...
                $previous = $order->returns->reject(fn ($return) => in_array($return->status, [OrderReturnStatus::DRAFT, OrderReturnStatus::CANCELLED], true))->flatMap(fn ($return) => $return->items)->where('order_item_id', $orderItem->id);
            ...
            $manualReturnAmount = isset($data['return_amount']) ? round((float) $data['return_amount'], 2) : null;
            $meta = $manualReturnAmount !== null ? ['return_amount' => $manualReturnAmount] : [];
            $orderReturn = OrderReturn::create([
                'code' => $this->codeGenerator->generate(OrderReturn::class),
                'order_id' => $order->id, 'customer_id' => $order->customer_id, 'employee_id' => $employee->id,
                'status' => OrderReturnStatus::DRAFT, 'description' => $data['description'] ?? null, 'meta' => $meta ?: null,
            ]);
            ...
            foreach ($data['items'] as $itemData) {
                ...
                $orderReturn->items()->create([
                    'order_item_id' => $orderItem->id, 'product_id' => $orderItem->product_id,
                    'quantity' => $quantity, 'free_quantity' => $freeQuantity,
                    'unit_price' => (float) $orderItem->unit_price,
                    'total_price' => round(($quantity - $freeQuantity) * (float) $orderItem->unit_price, 2),
