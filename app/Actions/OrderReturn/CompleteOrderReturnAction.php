...
class CompleteOrderReturnAction {
    ...
    public function execute(OrderReturn $orderReturn): OrderReturn {
        return DB::transaction(function () use ($orderReturn) {
            ...
            if($orderReturn->status!==OrderReturnStatus::CONFIRMED) throw new BusinessRuleException('فقط مرجوعی در وضعیت confirmed قابل تکمیل است.');
            if($orderReturn->items->isEmpty()) throw new BusinessRuleException('مرجوعی باید حداقل یک آیتم داشته باشد.');
            ...
            foreach($orderReturn->items as $item){
                if($item->allocations->isEmpty()) throw new BusinessRuleException('برای تمام اقلام مرجوعی باید تخصیص انبار ثبت شود.');
                $allocatedQuantity=$item->allocations->sum(fn($allocation)=>(int)$allocation->quantity);
                if($allocatedQuantity!==(int)$item->quantity) throw new BusinessRuleException('مجموع تخصیص‌های انبار با مقدار مرجوعی برابر نیست.');

                foreach($item->allocations as $allocation){
                    $batch=InventoryBatch::query()->lockForUpdate()->findOrFail($allocation->inventory_batch_id);
                    if((int)$batch->product_id!==(int)$item->product_id) throw new BusinessRuleException('Batch انتخاب‌شده متعلق به محصول مرجوعی نیست.');
                    $quantity=(int)$allocation->quantity;
                    if($quantity<=0) throw new BusinessRuleException('مقدار تخصیص انبار باید بیشتر از صفر باشد.');
                    $batch->quantity += $quantity; $batch->save();
                    InventoryMovement::create(['inventory_batch_id'=>$batch->id,'type'=>InventoryMovementType::IN,'quantity'=>$quantity,'reason'=>'order_return','description'=>$orderReturn->description,'moved_at'=>now()]);
                }
            }
            ...
            if($invoiceSubtotal<=0||$invoiceTotal<=0) throw new BusinessRuleException('مبالغ فاکتور برای محاسبه مرجوعی معتبر نیستند.');

            $calculatedReturnAmount=round(($returnAmount/$invoiceSubtotal)*$invoiceTotal,2);
            $previousReturnCredit=(float)CustomerTransaction::query()->where('customer_id',$orderReturn->customer_id)->whereHas('orderReturn',function($query)use($orderReturn){
                $query->where('order_id',$orderReturn->order_id)->where('status',OrderReturnStatus::COMPLETED)->whereKeyNot($orderReturn->id);
