...
{
    public function generate(CodeGeneratorData|string $config): string
    {
        if (is_string($config)) {
            $config = $config::codeGenerator();
        }
        ...
        return DB::transaction(function () use ($config, $now) {
            $sequence = Sequence::query()
                ->where('sequence_key', $config->sequence_key)
                ->lockForUpdate()
                ->first();
    ...
    {
        if ($reset === SequenceResetType::NONE || !$sequence->last_generated_at) {
            return;
        }
