...
class Visit extends BaseModel
{
    ...
    public const DEFAULT_RELATIONS = ['doctor', 'employee', 'samples.product'];
    public const SEARCHABLE = ['purpose', 'description'];
    public const SORTABLE = ['visit_date', 'created_at'];

    protected $fillable = [
        'client_operation_id', 'doctor_id', 'employee_id', 'status', 'visit_date',
        'latitude', 'longitude', 'location_accuracy', 'location_captured_at',
        'purpose', 'description', 'meta',
    ];
    ...
    public function doctor(): BelongsTo { return $this->belongsTo(Doctor::class)->withTrashed(); }
    public function employee(): BelongsTo { return $this->belongsTo(Employee::class)->withTrashed(); }
    public function samples(): HasMany { return $this->hasMany(Sample::class); }
