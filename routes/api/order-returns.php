...
Route::prefix('order-returns')->name('order-returns.')->group(function () {
    ...
    Route::post('/{orderReturn}/receive', [OrderReturnController::class, 'receive'])->name('receive');
});

Route::post('/order-return-items/{orderReturnItem}/allocate', [OrderReturnController::class, 'allocate'])->name('order-return-items.allocate');
